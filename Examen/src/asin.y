...
%token READ_ PRINT_ IF_ FOR_ TRUE_ FALSE_ ELSE_ EXCEPT_
...
instruccionIteracion
    ...
    expresionOpcional PUNTOCOMA_ EXCEPT_ expresion PARC_ 
        {
		if ($14.tipo != T_LOGICO)
            {
			yyerror("La Guarda del bucle for tiene que ser logica");
		    }
        //FALSO = SKIP Intruccion del cuerpo
		emite(EIGUAL, crArgPos($14.pos), crArgEnt(FALSE), crArgEtq($<cent>5));
        //TRUE saltar a = expresionOpcional_2
		emite(EIGUAL, crArgPos($14.pos), crArgEnt(TRUE), crArgEtq($<cent>10));
